...
-record(contract, {
        ...
        referrer_ids :: [aec_id:id()],%% List of contracts depending on this contract
        deposit      :: amount()
    }).
...
-spec referrer_ids(contract()) -> [id()].
referrer_ids(#contract{referrer_ids = ReferrerIds}) ->
    ReferrerIds.

-spec referrer_pubkeys(contract()) -> [pubkey()].
